...
def setup_windows_scheduler(config_path, python_executable):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # 检查是否是权限问题
            if (
                "7042" in result.stderr
                or "Access is denied" in result.stderr
                or "拒绝访问" in result.stderr
            ):
...
def setup_linux_cron(config_path, python_executable):
    ...
    try:
        ...
        if not cron_location:
            print(
                "无法定位系统 cron 配置文件。请确认 cron 服务已安装 (如 'cron', 'cronie' 或 BusyBox crond)。"
            )
            return False
        ...
        try:
            ...
            print(
                f"权限错误: 无法写入 {cron_location} 或其子文件。请尝试使用 'sudo' 运行此脚本。"
            )
            return False
        except Exception as e:
    ...
    try:
        ...
        if len(new_lines) < len(lines):
            ...
            try:
                ...
                print(
                    f"[错误] 权限不足，无法写入 {old_cron_path}。请使用 'sudo' 权限运行此脚本以更新定时任务。"
                )
                return False
            except Exception as e:
